  ...
  try {
    ...
    if (!["food", "package", "travel"].includes(rideType)) {
      return res.status(400).json({
        message:
          "Invalid ride type. It must be one of: food, package, or travel.",
      });
    }
  ...
  try {
    ...
    if (id) {
      ...
      if (!ride) {
        return res
          .status(404)
          .json({ message: `Ride with id ${id} not found` });
      }
...
export const getRideOffers = async (req, res) => {
  ...
  try {
    const rideOffers = await Ride.find({
      client: clientId,
      status: "placed",
    }).populate("driver service");
    res.status(200).json({ success: true, rideOffers });
  } catch (error) {
    res
      .status(500)
      .json({ success: false, message: "Error fetching ride offers" });
  }
};
export const updateRide = async (req, res) => {
  try {
    const updatedRide = await Ride.findByIdAndUpdate(req.query.id, req.body, {
      new: true,
    });
    if (!updatedRide) {
      return res
        .status(404)
        .json({ message: `Ride with id ${req.query.id} not found` });
    }
    res
      .status(200)
...
export const respondToRideOffer = async (req, res) => {
  ...
  try {
    ...
    if (!ride) {
      return res
        .status(404)
        .json({ success: false, message: "Ride offer not found" });
    }
    ...
    res.status(200).json({
      success: true,
      message: `Ride offer ${action}ed successfully`,
      ride,
    });
  } catch (error) {
    res
      .status(500)
      .json({ success: false, message: "Error handling ride offer" });
  }
};
...
export const getDriverRideStats = async (req, res) => {
  ...
  try {
    ...
    const stats = await Ride.aggregate([
      ...
      }, // Filter rides by driver and completed status
      {
        $group: {
          _id: "$driver",
    ...
    res
      .status(500)
      .json({ message: "Failed to retrieve driver ride statistics", error });
  }
};
export const getRidesByUser = async (req, res) => {
  try {
    ...
    if (!driverId && !clientId) {
      return res
        .status(400)
        .json({ message: "Driver ID or Client ID is required" });
    }
    ...
    if (!rides || rides.length === 0) {
      return res.status(404).json({
        message: `No rides found for ${
          driverId ? `driver` : `client`
        } with ID ${driverId || clientId}`,
      });
    }
